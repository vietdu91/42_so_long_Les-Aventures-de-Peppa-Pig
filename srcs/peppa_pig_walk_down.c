...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/12 11:45:27 by emtran            #+#    #+#             */
/*   Updated: 2021/11/15 11:45:41 by emtran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	to_down_for_bacons(t_game *game)
{
	...
	if (game->map->super_peppa_sale == 0)
		fried_bacon_to_down(game);
	else
		caramalized_bacon_down_like_gros_degueu(game);
}
...
void	peppa_walks_down(t_game *game)
{
	...
	!= '1' && time_to_play_with_george(game, 4) != 1)
	{
		if (game->map->mappy[game->map->pos_y_play + 1][game->map->pos_x_play] \
		== 'C')
			to_down_for_bacons(game);
		...
		{
			to_down_to_be_sale(game);
			vive_la_gadou();
		}
		else if (game->map->super_peppa_sale == 1)
			to_down_to_be_continued_sale(game);
		else
			peppa_is_downly_a_good_pig_girl(game);
		game->map->pos_y_play++;
